...
public class AVLTreetheoHangCard {
    ...
    private AVLNode insertRecursive(AVLNode node, GraphicsCard card) {
        ...
        int cmp = card.manufacturer.compareToIgnoreCase(node.manufacturer);
        if (cmp < 0)
        ...
        updateHeight(node);
        return balance(node, card.manufacturer);
    }
    ...
    private AVLNode balance(AVLNode node, String key) {
        int cmpLeft = (node.left != null) ? key.compareToIgnoreCase(node.left.manufacturer) : 0;
        int cmpRight = (node.right != null) ? key.compareToIgnoreCase(node.right.manufacturer) : 0;
