...
public class AVLTreetheoGPUCard {
    ...
    private AVLNode insertRecursive(AVLNode node, GraphicsCard card) {
        ...
        int cmp = card.gpuChip.compareToIgnoreCase(node.gpuChip);
        if (cmp < 0)
        ...
        updateHeight(node);
        return balance(node, card.gpuChip);
    }
    ...
    private AVLNode balance(AVLNode node, String key) {
        int cmpLeft = (node.left != null) ? key.compareToIgnoreCase(node.left.gpuChip) : 0;
        int cmpRight = (node.right != null) ? key.compareToIgnoreCase(node.right.gpuChip) : 0;
